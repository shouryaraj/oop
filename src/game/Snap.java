...
public class Snap extends Action{
	 ...
	 */
	public Snap(Location moveToLocation, String direction, String hotKey) {
	 ...
	 */
	public Snap(Location moveToLocation, String direction, Actor actor) {
		this.moveToLocation = moveToLocation;
		this.direction = direction;
		this.hotKey = "";
		this.player  = actor;
	 ...
	 * @return a description of the Action suitable for the menus
	 */
	@Override
	public String execute(Actor actor, GameMap map) {
		if((actor.hasSkill(PlayerEnum.HASGAUNTLET) && actor.hasSkill(PlayerEnum.HASPOWERSTONE) && actor.hasSkill(PlayerEnum.HASSOULSTONE) &&
				actor.hasSkill(PlayerEnum.HASREALITYSTONE)
				&& actor.hasSkill(PlayerEnum.HASMINDSTONE))){
			map.moveActor(actor, moveToLocation);
			
			actor.addSkill(PlayerEnum.HASSUPERPOWER);
			// Adding new Actors on the ThanosWOrld
		     
		
			
			return "SWAPPED!" +  " and Thanos is dead";
		}
		else {
			return "You need Power IRON MAN!";
		}
	}
	...
	public String menuDescription(Actor actor) {
		return actor + " Swap to eliminate the 50% population ";
	}
